...
bool dfs(const vector<P>& hole, const vector<vector<pair<int, int>>>& graph, int x, int broken_e, const vector<pair<int, int>>& edge) {
    if (counter % 100000 == 0) {
        fprintf(stderr, "%lld\n", counter);
        fflush(stderr);
    }
    ...
    if (x == order.size()) {
        ...
        if (order[x] == order.size() - 1) {
            ...
            for (int i = 0; i < available.size(); i++) {
                ...
                target[order[x]] = i;
                if (dfs(hole, graph, x + 1, broken_e, edge)) return true;
            }
        } else {
            for (int i = 0; i < available.size(); i++) {
                bool ok = true;
                for (const pair<int, int>& p : graph[order[x]]) {
	                if (p.second == broken_e) {
		                // broken
		                continue;
	                } else {
...
int main(int argc, char* argv[]) {
    ...
    order.push_back(graph.size());
    
    if (argc >= 3) min_dislike = atol(argv[2]);

    for (int i = 0; i < edge.size(); i++) {
      dfs(hole, new_graph, 0, i, edge);
    }
