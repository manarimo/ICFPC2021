...
class simulated_annealing {
    public:
    simulated_annealing(double start_temp);
    inline double get_time();
    inline bool end();
...
simulated_annealing::simulated_annealing(double start_temp) {
...
int main(int argc, char* argv[]) {
    ...
    while (unchanged < 5) {
        ...
        double penalty_vertex = 0;
        double penalty_edge = 0;
        double penalty_length = 0;
        
        simulated_annealing sa(random::get(10, 1000));
        number best_dislike = dislike;
        ...
        if (best_dislike < start_dislike) {
            unchanged = 0;
            origin = best_figure;
            fprintf(stderr, "%lld -> %lld\n", start_dislike, best_dislike);
            fflush(stderr);
        } else {
            unchanged++;
    ...
    problem.output(origin);
    if (argc >= 2) output_svg(argv[1], hole, edge, origin);
