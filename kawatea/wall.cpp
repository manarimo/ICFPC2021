...
bool check(const vector<P>& hole) {
    ...
    for (int i = 0; i < hole.size(); i++) {
        ...
        for (int j = 0; j < order.size(); j++) {
            if (target[j] < 0) {
                min_p = min(min_p, d(hole[i], unavailable[-target[j] - 1]));
            } else {
    ...
    for (int i = 0; i < order.size(); i++) {
        ...
        if (target[i] < 0) {
            fprintf(stderr, "[%lld, %lld]", unavailable[-target[i] - 1].X, unavailable[-target[i] - 1].Y);
        } else {
...
bool dfs(const vector<P>& hole, const vector<vector<pair<int, int>>>& graph, int x, bool wall_used) {
    if (counter % 100000 == 0) {
        fprintf(stderr, "%lld\n", counter);
        fflush(stderr);
    }
    ...
    if (x == order.size()) {
        ...
        if (x + 1 < order.size() || wall_used) {
            for (int i = 0; i < available.size(); i++) {
                bool ok = true;
                for (const pair<int, int>& p : graph[order[x]]) {
                    if (target[p.first] >= 0 && !edge_inside[i][target[p.first]]) {
                        ok = false;
                        break;
                    }
                    if (target[p.first] < 0) {
                        number dist = d(unavailable[-target[p.first] - 1], available[i]);
                        if (dist < min_len[p.second] || max_len[p.second] < dist) {
                            ok = false;
                            break;
                        }
                    } else if (dist[i][target[p.first]] < min_len[p.second] || max_len[p.second] < dist[i][target[p.first]]) {
                        ok = false;
                        break;
                    }
                }
                if (ok) {
                    target[order[x]] = i;
                    if (dfs(hole, graph, x + 1, wall_used)) return true;
                }
            }
        }
        if (!wall_used) {
            ...
            for (int i = 0; i < unavailable.size(); i++) {
                for (const pair<int, int>& p : graph[order[x]]) {
                    number dist = d(unavailable[i], available[target[p.first]]);
                    if (dist < min_len[p.second] || max_len[p.second] < dist) {
                        ok = false;
                        break;
                    }
                ...
                if (ok) {
                    target[order[x]] = - i - 1;
                    if (dfs(hole, graph, x + 1, true)) return true;
                }
            }
        }
        return false;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < n; i++) {
        if (best_target[i] < 0) {
            best_figure[i] = unavailable[-best_target[i] - 1];
        } else {
            best_figure[i] = available[best_target[i]];
        }
    }
    output(best_figure);
